...
| **路由器检测** | 自动填入当前网关，通过 SSH 读取分区并给出匹配建议；可随时跳过。 |
...
| SSH 检测 | IP 地址、端口与密码登录；RSA / ECDSA 服务器，AX6000 分区判断 |

已是 stock 的镜像、U-Boot mod 和其他机型暂不支持转换。Linux / macOS 暂未支持。

检测仅读取信息，不修改路由器，也不限制转换。分区匹配不代表已验证引导程序或完整刷机兼容性。
